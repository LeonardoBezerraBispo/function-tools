...
public class FunctionPadronizadaGET {
    ...
    @FunctionName("nome-da-funcao-aqui")
    /*
    ...
    */ 
    ...
    public HttpResponseMessage run(@HttpTrigger(
        ...
        try {
            ...
            // Quando queremos pegar um paramêtro Query utilizamos desta forma
            String Query = request.getQueryParameters().get("aqui-é-o-nome-do-paramêtro");
            String queryString = "";
            
            // Utilizamos essa condição para incluir nos padrões, que são incluir o "?" 
            if (Query != null) {
                queryString += ((queryString == "?") ? "nome-do-parametro-no-backend=" + Query
                        : "&nome-do-parametro-no-backend=" + Query);
            }

            HttpRequest requestPega = HttpRequest.newBuilder()
                    .uri(new URI("aqui é a url" + queryString))
